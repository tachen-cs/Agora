...
void SimpleClientMac::update_tx_buffer(gen_tag_t tag)
{
    ...
    for (size_t i = 0; i < cfg->mac_fragments; i++) {
        ...
        printf("time %0.2f sending packet for sequence %d, fragment %d, stream %d, bytes "
               "%zu\n",
            get_time(), pkt->sequence_id, pkt->fragment_id, pkt->stream_id,
            cfg->mac_fragment_payload_length);

        for (size_t i = 0; i < cfg->mac_fragment_payload_length; i++) {
